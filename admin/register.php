  ...
  if (empty($username) || empty($password) || empty($confirm_password)) {
    ...
    if (!$stmt) {
      ...
      if ($result->num_rows > 0) {
        ...
        // Hash password dan masukkan admin baru
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $stmt = $conn->prepare("INSERT INTO admin (name, username, password) VALUES (?, ?, ?)");
        if (!$stmt) {
          $error = 'Kesalahan query INSERT: ' . $conn->error;
        } else {
          $stmt->bind_param("sss", $name, $username, $hashed_password);
          if ($stmt->execute()) {
            $success = 'Akun admin berhasil dibuat. Silakan <a href="login.php" class="login-link">login</a>.';
          } else {
...
<head>
  ...
  <title>Daftar Admin - Sistem Presensi MIDU</title>
  ...
  <div class="register-container">
    <div class="card">
      <div class="card-header">
        ...
        <h4 class="mb-0">Daftar Admin - Sistem Presensi MIDU</h4>
      </div>
      <div class="card-body">
        <?php if (!empty($error)): ?>
          ...
          <div class="mb-3">
            ...
            <div class="input-group">
              ...
              <input type="password" name="confirm_password" id="confirm_password" class="form-control" required
                     placeholder="Ulangi password" aria-describedby="confirmPasswordHelp">
              <span class="input-group-text show-password" onclick="togglePassword('confirm_password', 'toggleConfirmIcon')">
                <i class="fas fa-eye" id="toggleConfirmIcon"></i>
              </span>
            </div>
          </div>
          <button type="submit" name="register" class="btn btn-primary w-100" id="registerBtn">Daftar</button>
